...
class ExpresspayEposPayment extends Payment implements \Commerce\Interfaces\Payment
{
    public function __construct($modx, array $params = [])
    {
        parent::__construct($modx, $params);
        $this->lang = $modx->commerce->getUserLanguage('expresspay_epos');
    }
    ...
    public function getMarkup()
    {
        ...
        if ($this->getSetting('isTest')) {
            $out[] = $this->lang['expresspay_epos.test_mode'];
        }

        if (empty($this->getSetting('serviceId'))) {
            $out[] = $this->lang['expresspay_epos.error_empty_serviceId'];
        }

        if (empty($this->getSetting('token'))) {
            $out[] = $this->lang['expresspay_epos.error_empty_token'];
        }
    ...
    public function getPaymentLink()
    {
        ...
        if (isset($response['Errors'])) {
            ...
            $epos_code  = $this->getSetting('eposCode') . "-";
            $epos_code  .= "1-";
            $epos_code .= $order['id'];
    ...
    private function log($name, $type, $message)
    {
        ...
        $log_url .= '/express-pay-epos' . date('Y.m.d') . '.log';
